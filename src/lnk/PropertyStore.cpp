...
static std::string FormatFileTime(uint64_t ft) {
    ...
    if (t == 0) {
        if (ft == 0) return "01/01/1601 00:00:00";
        return "";
    }
    struct tm tm = {};
    gmtime_r(&t, &tm);
    ...
    while (shellPropertyIndex < rawBytes.size()) {
        ...
        if (isNamed) {
            ...
            while (sheetIndex < sheetListBytes.size()) {
                ...
                size_t slotStart = sheetIndex;
                size_t recordEnd = slotStart + valueSize;
                if (recordEnd > sheetListBytes.size()) recordEnd = sheetListBytes.size();
                sheetIndex += 4;

                if (sheetIndex + 4 > recordEnd) break;
                int32_t nameSize = static_cast<int32_t>(ReadUInt32LE(sheetListBytes.data(), sheetIndex));
                sheetIndex += 4;

                if (sheetIndex >= recordEnd) break;
                sheetIndex += 1; // reserved

                std::string propertyName;
                if (nameSize > 0 && sheetIndex + nameSize <= recordEnd) {
                    propertyName = ReadUnicodeStringFromBytes(sheetListBytes.data(), sheetIndex, nameSize);
                    sheetIndex += nameSize;
                } else if (nameSize > 0) {
                    break;
                }
                ...
                size_t valueMaxLen = (recordEnd > sheetIndex) ? (recordEnd - sheetIndex) : 0;
                std::string value = ProcessNamedValue(sheetListBytes.data(), sheetIndex, namedType, valueMaxLen);
                std::string desc = propertyName; // C# uses propertyName as description for named properties
                sheet.properties.emplace_back(propertyName, desc, value);

                // C# VT_BLOB hack: Skip(0x69) from start of propertyValue.Value and parse nested PropertyStore
                if (namedType == 0x0041 && slotStart + 0x69 < recordEnd) {
                    try {
                        std::vector<uint8_t> nestedBytes(sheetListBytes.begin() + slotStart + 0x69,
                                                          sheetListBytes.begin() + recordEnd);
                        PropertyStore nested(nestedBytes);
                        for (const auto& nsheet : nested.sheets) {
                            for (const auto& nprop : nsheet.properties) {
                                sheet.properties.emplace_back(std::get<0>(nprop), std::get<1>(nprop), std::get<2>(nprop));
                            }
                        }
                    } catch (...) {
        ...
        // Terminator check: 4 bytes must be 0 after property loop
        if (sheetIndex + 4 <= sheetListBytes.size()) {
            if (ReadUInt32LE(sheetListBytes.data(), sheetIndex) != 0) {
                // Non-zero terminator — sheet may be malformed, but continue
            }
            sheetIndex += 4;
        }
...
// VT type constants (kept for reference; functions now use literal hex values to exactly match C#)
...
static std::string ProcessNamedValueInternal(const uint8_t* data, size_t& idx, uint16_t namedType, size_t maxLen);
static std::string ProcessNumericValueInternal(const uint8_t* data, size_t& idx, uint16_t numericType, size_t maxLen);

static std::string ProcessNamedValue(const uint8_t* data, size_t& idx, uint16_t namedType, size_t maxLen) {
    return ProcessNamedValueInternal(data, idx, namedType, maxLen);
}

static std::string ProcessNumericValue(const uint8_t* data, size_t& idx, uint16_t numericType, size_t maxLen) {
    return ProcessNumericValueInternal(data, idx, numericType, maxLen);
}
...
static std::string ProcessNamedValueInternal(const uint8_t* data, size_t& idx, uint16_t namedType, size_t maxLen) {
    switch (namedType) {
        ...
        case 0x0004: { // VT_R4
            if (maxLen >= 4) {
                uint32_t raw = ReadUInt32LE(data, idx);
                idx += 4;
                float f;
        ...
        case 0x0005: { // VT_R8
            if (maxLen >= 8) {
                uint64_t raw = ReadUInt64LE(data, idx);
                idx += 8;
                double d;
        ...
        case 0x0008: { // VT_BSTR (Unicode in original C#)
            if (maxLen < 4) return "";
            uint32_t strLen = ReadUInt32LE(data, idx);
            idx += 4;
            if (strLen > maxLen - 4) return "";
            std::string s = ReadUnicodeStringFromBytes(data, idx, strLen > 2 ? strLen - 2 : 0);
            idx += strLen;
            return s;
        }
        ...
        case 0x000b: // VT_BOOL
            if (maxLen >= 8) {
                idx += 8;
                return ReadUInt32LE(data, idx - 8) > 0 ? "True" : "False";
            }
            return "";
        case 0x0013: // VT_UI4
            if (maxLen >= 4) { idx += 4; return std::to_string(ReadUInt32LE(data, idx - 4)); }
        ...
        case 0x0015: // VT_UI8
            ...
            return "";
        case 0x0016: // Original C# reads Int32 here despite VT_I8 comment
            if (maxLen >= 4) { idx += 4; return std::to_string(static_cast<int32_t>(ReadUInt32LE(data, idx - 4))); }
            return "";
        case 0x0017: // Original C# reads UInt32 here
            if (maxLen >= 4) { idx += 4; return std::to_string(ReadUInt32LE(data, idx - 4)); }
            return "";
        case 0x001f: { // VT_LPWSTR
            ...
            if (byteCount > maxLen - 4) byteCount = static_cast<uint32_t>(maxLen - 4);
            std::string result = ReadUnicodeStringFromBytes(data, idx, byteCount);
            idx += byteCount;
            return result;
        }
        ...
        case 0x0041: { // VT_BLOB
            ...
            std::string result = "BLOB data: " + BytesToHex(data + idx, blobSize);
            idx += blobSize;
            return result;
        }
        case 0x0042: // VT_STREAM
            return "VT_STREAM not implemented (yet) See extension block section for contents for now";
        default:
            return "Unknown named property type: " + BytesToHex(reinterpret_cast<const uint8_t*>(&namedType), 2) + ", Hex data (after property type): " + BytesToHex(data + idx, maxLen);
    }
}
...
static std::string ProcessNumericValueInternal(const uint8_t* data, size_t& idx, uint16_t numericType, size_t maxLen) {
    switch (numericType) {
        case 0x0001: // VT_NULL
            return "Null";
        case 0x0002: // VT_UI2 (original C# reads UInt16 despite VT_I2 comment)
        ...
        case 0x0015: // VT_UI8
            if (maxLen >= 8) { idx += 8; return std::to_string(ReadUInt64LE(data, idx - 8)); }
            return "";
        case 0x001e: { // VT_LPSTR (original C# bug: uses Unicode)
            if (maxLen < 4) return "";
            uint32_t strLen = ReadUInt32LE(data, idx);
            idx += 4;
            if (strLen > maxLen - 4) return "";
            std::string s = ReadUnicodeStringFromBytes(data, idx, strLen);
            idx += strLen;
        ...
        case 0x0042: // VT_STREAM
            return "VT_STREAM not implemented";
        case 0x0048: { // VT_CLSID
            if (maxLen >= 16) {
                std::string g = GuidToString(data + idx);
                idx += 16;
                std::string folder = GetFolderNameFromGuid(g);
                return folder.empty() ? g : folder;
            }
            return "";
        }
        case 0x1011: // VT_VECTOR | VT_UI1
            return "VT_VECTOR data not implemented (yet) See extension block section for contents for now";
        case 0x101f: { // VT_VECTOR | VT_LPWSTR
            if (maxLen < 8) return "";
            idx += 4; // skip vector header
            uint32_t charCount = ReadUInt32LE(data, idx);
            idx += 4;
            if (charCount <= 0) return "";
            uint32_t byteCount = charCount * 2;
            if (byteCount > maxLen - 8) byteCount = static_cast<uint32_t>(maxLen - 8);
            std::string result = ReadUnicodeStringFromBytes(data, idx, byteCount);
            idx += byteCount;
            return result;
        }
        case 0x1048: // VT_VECTOR | VT_CLSID
            return "VT_VECTOR data not implemented (yet)";
        default:
            return "Unknown numeric property type: " + BytesToHex(reinterpret_cast<const uint8_t*>(&numericType), 2) + ", Hex data (after property type): " + BytesToHex(data + idx, maxLen);
    }
}
